...
:::list-table
   ...
   * - Object name (*<rootobject>.FaultMgmt.*)
     - Table size 
     - Content
     - Purpose and usage
   ...
   * - SupportedAlarm.{i}.
     ...
     - Defines all alarms that the CPE supports. *ReportedMechanism* defines how the alarm is to be handled within the CPE: 
       - 0 -- Expedited
       - 1 -- Queued
       - 2 -- Logged
       - 3 -- Disabled
   ...
   * - ExpeditedEvent.{i}.
     - Fixed
     - Dynamically updated
     - Contains all *"Expedited"* type alarm events since the last device initialization. This includes events that are already reported or not yet reported to the Controller. One entry exists for each event. In other words, raising and clearing of the same alarm are two separate entries. As the table size is fixed (vendor defined), new alarm event overwrites the oldest entry in FIFO fashion after the table becomes full.

   * - QueuedEvent.{i}.
     - Fixed
     - Dynamically updated
     - Contains all *"Queued"* type alarm events since the last device initialization. This includes events that are already reported or not yet reported to the Controller. One entry exists for each event. In other words, raising and clearing of the same alarm are two separate entries.
   ...
   * - CurrentAlarm.{i}.
     - Variable
     - Dynamically updated
     - Contains all the currently active alarms (i.e., outstanding alarms that are not yet cleared) since the last device initialization. When an outstanding alarm is cleared, that entry is deleted from this table. Therefore, only 1 entry exists for a given unique alarm.
   ...
   * - HistoryEvent.{i}.
     - Fixed
     - Dynamically updated
     - Contains all alarm events as a historical record keeping purpose. One entry exists for each event. In other words, raising and clearing of the same alarm are two separate entries.
