...
sub parse
{
    ...
    # find config file
    if ($configfile) {
        my ($dir, $file) = main::find_file($configfile, {predir => ''});
...
sub ignore
{
    ...
    # XXX commands and events are objects for this purpose
    return 1 if $node->{type} !~ /^(model|object)$/;

    # $i is depth (0=top, 1=model, 2=root)
...
sub check
{
    ...
    foreach my $group (@$groups) {
        ...
        foreach my $gpath (@$gpaths) {
            ...
            if ($path eq $fpath) {
                #main::tmsg "$gname: $path matches exactly";
                return $group;
            }
        }
    }
    ...
    foreach my $group (@$groups) {
        ...
        foreach my $cpath (@$cpaths) {
            ...
            foreach my $gpatt (@$gpatts) {
                ...
                if ($cpath =~ /$ppath/) {
                    #main::tmsg "$gname: $path matches pattern";
                    return $group;
                }
            }
        }
    ...
    #main::tmsg "$path doesn't match any group";
    return $groups->[0];
}
...
sub div_node
{
    ...
    if ($i == 3) {
        $curgroup = check($node);
        #main::tmsg "$path: switched to $curgroup->{name}";
...
<div class="border dashed color$n float">
  <div class="center clear gray moveup">$curgroup->{label}</div>
    ...
    $curgroup->{text} .= output $n, qq{
<div class="$docwidth border solid color$n float$class"$style>
...
sub div_post
{
    ...
    if ($i == 3) {
        ...
        #main::tmsg "$node->{path}: reverted to $curgroup->{name}";
    }
}
